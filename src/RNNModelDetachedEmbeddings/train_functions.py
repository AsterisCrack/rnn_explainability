...
from src.RNNModelDetachedEmbeddings.utils import calculate_accuracy
...
def train_torch_model(
    ...
    for epoch in range(epochs):
        ...
        for features, labels, text_len in train_dataloader:
            ...
            # TODO: Get the embedded features
            embedded_features: torch.Tensor = embedding(features)

            # TODO: Clear the gradients
            optimizer.zero_grad()

            # TODO: Forward pass (compute the model output)
            output = model(embedded_features, text_len)
        ...
        train_accuracy = calculate_accuracy(
            model, train_dataloader, embedding, device=device)
        val_accuracy = calculate_accuracy(
            model, val_dataloader, embedding, device=device)
...
def t_step(
    ...
    test_accuracy = calculate_accuracy(
        model, test_dataloader, embeddings, device=device)
