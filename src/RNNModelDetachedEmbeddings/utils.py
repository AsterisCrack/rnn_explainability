...
def calculate_accuracy(
    ...
    for texts, labels, lengths in dataloader:
        ...
        embedded_texts = embeddings(texts)

        # get the model's predictions
        predictions = model(embedded_texts, lengths)
...
def plot_accuracies(model: torch.nn.Module,
    ...
    last_train_accuracy = calculate_accuracy(
        model, train_dataloader, embeddings, device=device)
    last_val_accuracy = calculate_accuracy(
        model, val_dataloader, embeddings, device=device)
